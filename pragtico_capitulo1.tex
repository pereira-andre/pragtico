...
\midrule
Interface Web & Flask com templates Jinja2, HTML semântico, CSS modular (6 módulos temáticos) e JavaScript vanilla \\
Autenticação & Sessões Flask com bcrypt, perfis diferenciados de administrador, agente e piloto, proteção CSRF e rate limiting \\
Backend aplicacional & Flask com Blueprints, camada de serviços e validação centralizada, responsável pela lógica de negócio e gestão de estados \\
Base de dados operacional & Abstração de armazenamento com dois backends: ficheiros JSON locais (desenvolvimento) e PostgreSQL com psycopg (produção) \\
Pesquisa vetorial / RAG & Embeddings locais com sentence-transformers (all-MiniLM-L6-v2) e índice vetorial local, com fallback para embeddings via API \\
Assistente inteligente & LLM acedido por API, com Gemini como opção principal e OpenRouter como alternativa multi-modelo \\
Armazenamento documental & Repositório de documentação operacional, normativa e administrativa, com extração de texto e indexação automática \\
Fontes externas & Tabelas oficiais de marés em CSV, dados meteorológicos via WeatherAPI e dados AIS de navios \\
\bottomrule
\end{tabular}
\end{table}
...
\bibitem{flask}
Pallets Projects. (s.d.). \textit{Flask -- Web development, one drop at a time}. Disponível em: \url{https://flask.palletsprojects.com/}. Consultado em março de 2026.
...
\bibitem{openrouter}
OpenRouter. (s.d.). \textit{OpenRouter -- A unified interface for LLMs}. Disponível em: \url{https://openrouter.ai/}. Consultado em março de 2026.

\bibitem{sbert}
Reimers, N. \& Gurevych, I. (2019). \textit{Sentence-BERT: Sentence Embeddings using Siamese BERT-Networks}. Proceedings of EMNLP-IJCNLP 2019, 3982--3992.
...
\bibitem{psycopg}
psycopg. (s.d.). \textit{psycopg -- PostgreSQL database adapter for Python}. Disponível em: \url{https://www.psycopg.org/}. Consultado em março de 2026.
