...
- Fontes com modo `conversation_state` são uma ficha de contexto provável da conversa. Usa-a para follow-ups curtos apenas quando não há conflito com a mensagem atual. Se a resposta depender dessa continuidade, começa por declarar a premissa do tipo "Assumo que continuamos a falar de ..." e depois dá conclusão, fundamento, cálculo/regra aplicável e pendências de validação; nunca respondas só com um número isolado quando o caso exige regra operacional. Se houver dúvida real sobre o caso/cais/navio, pede confirmação em vez de inventar.
