...
def build_tuning_map_snapshot(
    *,
    ...
    auto_trust_positive = bool(settings.get("auto_trust_positive_feedback", True))
    require_admin_validation = bool(settings.get("require_admin_validation", False))
    positive_feedback_state = (
        "online"
        if auto_trust_positive and not require_admin_validation
        else "degraded"
        if auto_trust_positive and require_admin_validation
        else "offline"
    )
        ...
        _build_tuning_group(
            ...
            components=[
                ...
                _build_tuning_component(
                    ...
                    detail="Quando ligado, aprovações reforçam respostas futuras sem exigir correção textual.",
                    facts=[
                        f"Modo {'ligado' if auto_trust_positive else 'desligado'}",
                        "Só feedback de admin é reutilizado" if require_admin_validation else "Feedback aprovado pode ser reutilizado diretamente",
            ...
            components=[
                ...
                _build_tuning_component(
                    ...
                    facts=[
                        ...
                        f"require_admin_validation = {'on' if require_admin_validation else 'off'}",
                        "Correções não-admin ficam como guard/revisão, não como memória forte" if require_admin_validation else "",
