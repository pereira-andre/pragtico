...
class OperationalFlowTests(unittest.TestCase):
    ...
    def test_admin_event_reports_page_supports_review_edit_and_print(self) -> None:
        ...
        with app.app.test_client() as client:
            ...
            print_index = html.index("Imprimir PDF")
            delete_index = html.index("Apagar</button>")
            self.assertNotIn("Abrir texto", html)
            self.assertNotIn("Abrir foto", html)
            self.assertIn('class="mini-button secondary-button" form="event-report-print-form" id="event-report-print-selected"', html)
            self.assertLess(edit_index, print_index)
            self.assertLess(print_index, delete_index)
